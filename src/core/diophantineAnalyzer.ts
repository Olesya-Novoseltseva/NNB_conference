...
export function analyzeDiophantineInput(
    ...
    rep(
      ...
      [
        ...
        `Максимальная степень (по уравнениям): ${maxDeg}.`,
          "Этот блок описывает структуру задачи и помогает понять, какие методы ниже имеют строгие основания для запуска.",
      ],
    ),
  );
      ...
      if (res.ok) {
        ...
        reports.push(
          rep(
            ...
            [
              ...
              "Метод: правило Крамера с дополнительной проверкой целочисленности найденной точки.",
              res.note ?? "",
              `Проверка: в найденной точке обе левые части обращаются в ноль одновременно.`,
        ...
        reports.push(
          rep(
            ...
            true,
            [
              "Метод применим, потому что обе строки имеют линейный вид по двум переменным.",
              res.reason,
              "Вывод строгий: система в целых числах в текущей постановке не имеет решений.",
            ],
          ),
        );
        linearExactHandled = true;
      ...
      if (sol.hasSolution) {
        ...
        reports.push(
          rep(
            ...
            [
              ...
              "Метод: рекурсивная сборка частного решения через расширенный алгоритм Евклида.",
              "Показывается одна конкретная точка; параметрическое семейство решений остается за кадром.",
        ...
        reports.push(
          rep(
            ...
            true,
            [
              "Проверен необходимый критерий делимости gcd коэффициентов на правую часть.",
              sol.reason,
              "Вывод строгий: уравнение не имеет целочисленных решений.",
            ],
          ),
        );
      }
      ...
      reports.push(
        rep(
          ...
          [
            ...
            "Блок использует локальное необходимое условие: целое решение должно давать решение по модулю каждого простого p.",
      ...
      reports.push(
        rep(
          ...
          [
            ...
            "Если по конкретному p перебор усечен лимитом проверок, строгий отрицательный вывод по этому p не делается.",
            ...lines,
    ...
    reports.push(
      rep(
        ...
        [
          ...
          "Этот блок оценивает масштаб вычислений, а не решает задачу сам по себе.",
    ...
    reports.push(
      rep(
        ...
        [
          ...
          search.solutions.length > 0
            ? "Найденные точки являются корректными решениями в пределах выбранного окна."
            : "Отсутствие точек внутри окна не переносится автоматически на всё множество целых чисел.",
  ...
  const needsHilbert10BoundaryNote = !linearExactHandled && maxDeg >= 2;

  if (needsHilbert10BoundaryNote) {
    reports.push(
      rep(
        "h10",
        ...
        true,
        [
          "Для произвольного диофантова уравнения над Z не существует универсального алгоритма «да/нет за конечное время» (отрицательное решение 10-ой проблемы Гильберта + MRDP).",
